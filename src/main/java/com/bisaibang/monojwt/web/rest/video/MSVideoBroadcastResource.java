...
public class MSVideoBroadcastResource {
    ...
    public ResponseEntity<?> getAllVideo(@Valid Pageable page) throws URISyntaxException {
        ...
        return ResponseEntity.ok(videoBroadcastRepository.findAllByTypeAndTag("normal", "video", pageable));
    }

    //获取所有赛程对阵图以及video
    @RequestMapping(value = "/ms-video/get-all",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<?> getAll(@Valid Pageable page) throws URISyntaxException {
        List<Sort.Order> orders = new ArrayList<Sort.Order>();
        orders.add(new Sort.Order(Sort.Direction.DESC, "id"));
        Pageable pageable = new PageRequest(page.getPageNumber(), page.getPageSize(), new Sort(orders));
     ...
     * get video by tag
     */
    @GetMapping("/ms-video/get-video-by-tag/tag/{tag}")
    public ResponseEntity<?> getVideoByTag(@PathVariable String tag, @Valid Pageable pageable) throws URISyntaxException {
//        if (!msAuthority.isAdmin()) {
//            return ResponseEntity.ok(message("权限不足"));
//        }
        Page<VideoBroadcast> page = videoBroadcastRepository.findAllByTypeAndTag("normal", tag, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/ms-video/get-video-by-tag");
